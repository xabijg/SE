...
extern int division_ent(int dividendo, int divisor);
...
void lcd_display_dec(uint16_t value) {
    if (value > 9999) {
        ...
        temp = value / 1000;
        cociente = division_ent(value, 1000);
        resto = value - cociente * 1000;
        lcd_set(temp, 1);

        temp = resto / 100;
        cociente = division_ent(resto, 100);
        resto = resto - cociente * 100;
        lcd_set(temp, 2);

        temp = resto / 10;
        cociente = division_ent(resto, 10);
        resto = resto - cociente * 10;
        lcd_set(temp, 3);
